...
do
    ...
    co_1_2="saveConnec_1_2_InhWei_$i.spc"
    sed_param_learn="$sed_param_learn;s|%CO_1_2|$co_1_2|"
    sed_param_test="$sed_param_test;s|%CO_1_2|$co_1_2|"
    
    co_2_3="saveConnec_2_3_InhWei_$i.spc"
    sed_param_learn="$sed_param_learn;s|%CO_2_3|$co_2_3|"
    sed_param_test="$sed_param_test;s|%CO_2_3|$co_2_3|"

    poids_1_2="weights_1_2_InhWei_$i.spw"
    sed_param_learn="$sed_param_learn;s|%PDS_1_2|$poids_1_2|"
    sed_param_test="$sed_param_test;s|%PDS_1_2|$poids_1_2|"

    poids_2_3="weights_2_3_InhWei_$i.spw"
    sed_param_learn="$sed_param_learn;s|%PDS_2_3|$poids_2_3|"
    sed_param_test="$sed_param_test;s|%PDS_2_3|$poids_2_3|"

    # Paramètres à tester

    inh_weight="$i*25.0"                      # volt
    sed_param_learn="$sed_param_learn;s|%INH_W|$inh_weight|"
    sed_param_test="$sed_param_test;s|%INH_W|$inh_weight|"
    
    # Génération des sources

    sed -e $sed_param_learn learnST_US_gen.py >> learnST_US_InhWei_$i.py
    sed -e $sed_param_test testST_US_gen.py >> testST_US_InhWei_$i.py
    
    chmod +x learnST_US_InhWei_$i.py
    chmod +x testST_US_InhWei_$i.py

    # Exécution

    echo "Exécution learnST_US_InhWei_$i.py : "`date`

    message=`./learnST_US_InhWei_$i.py`
    
    echo -e "Résultats après apprentissage\nlearnST_US_InhWei_$i\n\n$message" >> resTestInhWei_$i.txt

    echo -e "\n\n#######################################################\n\n" >> resTestInhWei_$i.txt
    
    echo "Exécution testST_US_InhWei_$i.py : "`date`

    message=`./testST_US_InhWei_$i.py`

    echo -e "Résultats après tests\ntestST_US_InhWei_$i.py\n\n$message" >> resTestInhWei_$i.txt

    # Nettoyage

    rm learnST_US_InhWei_$i.py testST_US_InhWei_$i.py 
    rm saveConnec_1_2_InhWei_$i.spc saveConnec_2_3_InhWei_$i.spc 
    rm weights_1_2_InhWei_$i.spw weights_2_3_InhWei_$i.spw
    ...
    mv resTestInhWei_* "resTests/$dayDir/"
