...
do
    ...
    sed -e $sed_param_learn learnST_US_gen.py >> learnST_US_NOM_$i.py
    sed -e $sed_param_test testST_US_gen.py >> testST_US_NOM_$i.py
    
    chmod +x learnST_US_NOM_$i.py
    chmod +x testST_US_NOM_$i.py

    # Exécution

    echo "Exécution learnST_US_NOM_$i.py : "`date`

    message=`./learnST_US_NOM_$i.py`
    
    echo -e "Résultats après apprentissage\nlearnST_US_NOM_$i\n\n$message" >> resTestNOM_$i.txt

    echo -e "\n\n#######################################################\n\n" >> resTestNOM_$i.txt
    
    echo "Exécution testST_US_NOM_$i.py : "`date`

    message=`./testST_US_NOM_$i.py`

    echo -e "Résultats après tests\ntestST_US_NOM_$i.py\n\n$message" >> resTestNOM_$i.txt

    # Nettoyage

    rm learnST_US_NOM_$i.py testST_US_NOM_$i.py
    rm saveConnec_1_2_NOM_$i.spc saveConnec_2_3_NOM_$i.spc 
    rm weights_1_2_NOM_$i.spw weights_2_3_NOM_$i.spw
    ...
    mv resTestNOM_* "resTests/$dayDir/"
