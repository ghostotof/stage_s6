...
for repet in xrange(1,5):
    ...
    for id,h,i,a,c in tab:
        ...
        if oldI != i:
            ...
            if h:
                nbImagesH += 1
                # for y in xrange(0, nbCol):
                #     teachTimes.append((0, nIm * t_pres + y * t_col))
                teachTimes.append((0, nIm * t_pres))
                # Sinon ce n'est pas un visage souriant
            else:
                nbImagesN += 1
                # for y in xrange(0, nbCol):
                #     teachTimes.append((1, nIm * t_pres + y * t_col))
                teachTimes.append((1, nIm * t_pres))
...
tau_3 = basePeriod * (1.0/3.0)
# refractory_3 = 0.5 * basePeriod
refractory_3 = 0.5 * t_pres
...
inhib_couche1[len(couche1), len(couche1) - 2] = inhib_weight
...
inhib_couche2[len(couche2), len(couche2)-1] = inhib_weight
